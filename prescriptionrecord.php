    ...
    <table class="table table-striped">
      <tbody>
        ...
        <tr>
          ...
          <td width="66%">
		  <select class="form-control show-tick" name="serviceid" id="serviceid" onchange="loadofferedservice(this.value)">
		  <option value="">Select Service</option>
		  <?php
		$sqlofferedservice ="SELECT * FROM offeredservice WHERE status='Active'";
		$qsqlofferedservice = mysqli_query($con,$sqlofferedservice);
		while($rsofferedservice = mysqli_fetch_array($qsqlofferedservice))
		{
			echo "<option value='$rsofferedservice[serviceid]'>$rsofferedservice[servicename] ( ₱ $rsofferedservice[servicecost] )</option>";
		}
		?>
		  </select>
		  </td>
        ...
        <tr>
          <td>Cost</td>
          <td><input class="form-control" type="text" name="cost" id="cost" value="<?php echo $rsofferedservice['servicecost']; ?>" readonly style="background-color:pink;" /></td>
        </tr>
        <tr>
          <td>Unit</td>
    ...
    <table class="table table-hover table-striped">
      <tbody>
        <tr>
         ...
         <?php
		 $gtotal=0;
		$sql ="SELECT * FROM prescription_records LEFT JOIN offeredservice on prescription_records.service_name=offeredservice.serviceid WHERE prescription_id='$_GET[prescriptionid]'";
		$qsql = mysqli_query($con,$sql);
		while($rs = mysqli_fetch_array($qsql))
		{
        echo "<tr>
...
function loadofferedservice(serviceid)
{
	...
	xmlhttp.open("GET","ajaxmedicine.php?serviceid="+serviceid,true);
	xmlhttp.send();
}
...
function validateform()
{
	...
	else if(document.frmpresrecord.offeredservice.value == "")
	{
		alert("Medicine field should not be empty..");
		document.frmpresrecord.offeredservice.focus();
		return false;
	}
